...
class AutomaticLeaveAllocation(models.Model):
    ...
    @api.onchange('fiscal_start_date')
    def onchange_fiscal_start_date(self):
        for line in self:
            if line.fiscal_start_date:
                line.update({
                           'fiscal_year': line.fiscal_start_date.year, 
                           }) 
                ...
                if al.employee_timeoff_ids:
                    for record in al.employee_timeoff_ids:
                        if record.permanent > 0:
                            ...
                            if not record.timeoff_type_id.is_annual_leave == True:
                                new_permanent_employees = self.env['hr.employee'].search(
                                    [('emp_type', '=', 'permanent'), ('company_id', '=', al.company_id.id),
                                     ('date', '>=', fiscal_start_date)])
                                for new_permanent_emp in new_permanent_employees:
                                    if new_permanent_emp.confirm_date.year<int(al.fiscal_year):
                                        leave_obj = leave_alloc_obj.create({
                                            'name': 'Auto-Leave Allocation for ' + al.name + ' with Emp Type Permanent',
                                            'holiday_status_id': record.timeoff_type_id.id,
                                            'employee_id': new_permanent_emp.id,
                                            'state': 'confirm',
                                            'number_of_days_display': record.permanent,
                                            'number_of_days': record.permanent,
                                        })
                        ...
                        if record.contractor > 0:
                            ...
                            for contractor_employee in contractor_employees:
                                ...
                                if leave_obj.state == 'validate':
                                    ...
                                    leave_obj.action_validate()
                            if not record.timeoff_type_id.is_annual_leave == True:
                                new_contractor_employees = self.env['hr.employee'].search(
                                    [('emp_type', '=', 'contractor'), ('company_id', '=', al.company_id.id),
                                     ('date', '>=', fiscal_start_date)])
                                for newcontractor_employee in new_contractor_employees:
                                    if new_permanent_emp.confirm_date.year < int(al.fiscal_year):
                                        leave_obj = leave_alloc_obj.create({
                                            'name': 'Auto-Leave Allocation for ' + al.name + ' with Emp Type Contractor',
                                            'holiday_status_id': record.timeoff_type_id.id,
                                            'state': 'confirm',
                                            'employee_id': newcontractor_employee.id,
                                            'number_of_days_display': record.contractor,
                                            'number_of_days': record.contractor,
                                        })
                                        leave_obj.update({
                        ...
                        if record.freelancer > 0:
                            ...
                            for freelancer_employee in freelancer_employees:
                                ...
                                if leave_obj.state == 'validate':
                                    ...
                                    leave_obj.action_validate()
                            if not record.timeoff_type_id.is_annual_leave == True:
                                new_freelancer_employees = self.env['hr.employee'].search(
                                    [('emp_type', '=', 'freelancer'), ('company_id', '=', al.company_id.id),
                                     ('date', '>=', fiscal_start_date)])
                                for new_freelancer_emp in new_freelancer_employees:
                                    if new_permanent_emp.confirm_date.year < int(al.fiscal_year):
                                        leave_obj = leave_alloc_obj.create({
                                            'name': 'Auto-Leave Allocation for ' + al.name + 'with Emp Type Freelancer',
                                            'holiday_status_id': record.timeoff_type_id.id,
                                            'employee_id': new_freelancer_emp.id,
                                            'state': 'confirm',
                                            'number_of_days_display': record.freelancer,
                                            'number_of_days': record.freelancer,
                                        })
                        ...
                        if record.intern > 0:
                            intern_employees = self.env['hr.employee'].search(
                                [('emp_type', '=', 'intern'), ('company_id', '=', al.company_id.id),
                                 ('date', '<=', fiscal_start_date)])
                            for intern_emp in intern_employees:
                                leave_obj = leave_alloc_obj.create({
                                    'name': 'Auto-Leave Allocation for ' + al.name + 'with Emp Type Intern',
                                    'holiday_status_id': record.timeoff_type_id.id,
                                    'employee_id': intern_emp.id,
                                    'state': 'confirm',
                            ...
                            newintern_employees=self.env['hr.employee'].search([('emp_type','=','intern'),('company_id','=', al.company_id.id),('date','>=', fiscal_start_date)])
                            for new_intern_emp in newintern_employees:
                                if new_permanent_emp.confirm_date.year < int(al.fiscal_year):
                                    leave_obj = leave_alloc_obj.create({
                                        'name': 'Auto-Leave Allocation for ' + al.name + 'with Emp Type Intern',
                                        'holiday_status_id':  record.timeoff_type_id.id,
                                        'employee_id': new_intern_emp.id,
                                        'state': 'confirm',
                                        'number_of_days_display': record.intern,
                                        'number_of_days': record.intern,
                                    })
                        ...
                        if record.part_time > 0:
                            ...
                            for part_time_employee in part_time_employees:
                                ...
                                if leave_obj.state == 'validate':
                                    ...
                                    leave_obj.action_validate()
                            if not record.timeoff_type_id.is_annual_leave == True:
                                newpart_time_employees = self.env['hr.employee'].search(
                                    [('emp_type', '=', 'part_time'), ('company_id', '=', al.company_id.id),
                                     ('date', '>=', fiscal_start_date)])
                                for newpart_time_emp in newpart_time_employees:
                                    if new_permanent_emp.confirm_date.year < int(al.fiscal_year):
                                        leave_obj = leave_alloc_obj.create({
                                            'name': 'Auto-Leave Allocation for ' + al.name + 'with Emp Type Part Time',
                                            'holiday_status_id': record.timeoff_type_id.id,
                                            'employee_id': newpart_time_emp.id,
                                            'state': 'confirm',
                                            'number_of_days_display': record.part_time,
                                            'number_of_days': record.part_time,
                                        })
                        ...
                        if record.project_based > 0:
                            ...
                            for project_based_employee in project_based_employees:
                                ...
                                if leave_obj.state == 'validate':
                                    ...
                                    leave_obj.action_validate()
                            if not record.timeoff_type_id.is_annual_leave == True:
                                newproject_based_employees = self.env['hr.employee'].search([
                                    ('emp_type', '=', 'project_based'), ('company_id', '=', al.company_id.id),
                                    ('date', '>=', fiscal_start_date)])
                                for newproject_based_emp in newproject_based_employees:
                                    if new_permanent_emp.confirm_date.year < int(al.fiscal_year):
                                        leave_obj = leave_alloc_obj.create({
                                            'name': 'Auto-Leave Allocation for ' + al.name + 'with Emp Type Project Based',
                                            'holiday_status_id': record.timeoff_type_id.id,
                                            'state': 'confirm',
                                            'employee_id': newproject_based_emp.id,
                                            'number_of_days_display': record.project_based,
                                            'number_of_days': record.project_based,
                                        })
                                        leave_obj.update({
                        ...
                        if record.outsource > 0:
                            ...
                            for outsource_employee in outsource_employees:
                                ...
                                if leave_obj.state == 'validate':
                                    ...
                                    leave_obj.action_validate()
                            if not record.timeoff_type_id.is_annual_leave == True:
                                newoutsource_employees = self.env['hr.employee'].search(
                                    [('emp_type', '=', 'outsource'), ('company_id', '=', al.company_id.id),
                                     ('date', '>=', fiscal_start_date)])
                                for newoutsource_emp in newoutsource_employees:
                                    if new_permanent_emp.confirm_date.year < int(al.fiscal_year):
                                        leave_obj = leave_alloc_obj.create({
                                            'name': 'Auto-Leave Allocation for' + al.name + 'with Emp Type Out Source',
                                            'holiday_status_id': record.timeoff_type_id.id,
                                            'employee_id': newoutsource_emp.id,
                                            'state': 'confirm',
                                            'number_of_days_display': record.outsource,
                                            'number_of_days': record.outsource,
                                        })
