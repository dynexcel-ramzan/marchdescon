...
class ApprisalFeedback(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(1,6, 'Performance Period',bold)
        ...
        for line in lines: 
            ...
            sheet.write(row, 2, line.name.parent_id.name, format1)
            sheet.write(row, 3, 'Active' if line.name.active==True else 'In-Active', format1)
            sheet.write(row, 4, line.name.work_location_id.name, format1)
            sheet.write(row, 5, line.name.department_id.name, format1)
            sheet.write(row, 6, str(financial_year), format1)  
            sheet.write(row, 7, str(appraisal_status), format1)
            sheet.write(row, 8, line.name.grade_type.name, format1) 
            sheet.write(row, 9,  str(employee_type), format1)  
            sheet.write(row, 10, line.name.job_id.name, format1)
            sheet.write(row, 11, line.name.grade_designation.name, format1) 
            sheet.write(row, 12, line.mid_year_date.strftime('%d-%b-%Y') if line.mid_year_date else '-', format1) 
            sheet.write(row, 13, line.end_year_date.strftime('%d-%b-%Y') if line.end_year_date else '-', format1) 
            sheet.write(row, 14, line.name.company_id.name, format1)
            row += 1           