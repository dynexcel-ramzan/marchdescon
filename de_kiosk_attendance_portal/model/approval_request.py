...
class ApprovalRequest(models.Model):
    ...
    wfh_id = fields.Many2one('hr.attendance.wfh', string='WFH', required=False)
    ...
    def action_refuse(self, approver=None):
        ...
        for approverid in self.approver_ids:
            if  approverid.status == 'refused': 
                self.wfh_id.sudo().action_refuse()
    ...
    def action_approve(self, approver=None):      
        ...
        if approver_count == 0:
            ...
            if tot_approver_count ==  tot_approved_count: 
                if self.wfh_id:
                    self.wfh_id.sudo().action_approve()       
        ...
        for request in self:
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    status = 'approved'
                    if request.wfh_id:
                        request.wfh_id.sudo().action_approve()
                    
                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.wfh_id:
                        request.wfh_id.sudo().action_refuse()    
                else:
                    status = 'pending'
            else:
                status = 'new'
...
class ApprovalApprover(models.Model):
    ...
    wfh_id = fields.Many2one(related='request_id.wfh_id')