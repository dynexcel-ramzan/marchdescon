...
class CreateKAttendance(http.Controller):
    ...
    def kattendance_create_checkin(self, **kw):
        ...
        att_exists = request.env['hr.attendance'].search([('employee_id','=', employee_id.id),('check_out','=',False),('att_date','=', fields.date.today()),('remarks','=','WFH(Work From Home)')], order='check_in desc', limit=1)
        if att_exists:
    ...
    def kattendance_create_checkout(self, **kw):
        ...
        employee_id = kattendance_page_content()['employees']
        att_date =  fields.date.today()
        att_exists = request.env['hr.attendance'].search([('employee_id','=', employee_id.id),('check_out','=',False),('att_date','=',att_date),('remarks','=','WFH(Work From Home)')], order="check_in asc" , limit=1)
