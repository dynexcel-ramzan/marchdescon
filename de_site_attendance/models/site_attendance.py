...
class SiteAttendnace(models.Model):
    ...
    def action_create_approval_request_site_attendance(self):
        ...
        request_list = []
        for line in self: 
            site_category=self.env['approval.category'].search([('name','=','Site Attendance'),('company_id','=', line.incharge_id.company_id.id)], limit=1)
            if not site_category:
                ...
                site_category = self.env['approval.category'].sudo().create(category)
            line.category_id= site_category.id   
            if site_category:
                request_list.append({
                        'name': line.incharge_id.name + ' Has Site Attendance Request Location ' + str(line.location_id.name)+ ' Date From ' + str(line.date_from.strftime('%d-%b-%Y'))+' '+' Date To '+str(line.date_to.strftime('%d-%b-%Y')),
                        'request_owner_id': line.incharge_id.user_id.id,
                        'category_id': line.category_id.id,
                        'site_id': line.id,
                        'reason': line.incharge_id.name + ' Has Site Attendance Request Location ' + str(line.location_id.name)+ ' Date From ' + str(line.date_from.strftime('%d-%b-%Y'))+' '+' Date To '+str(line.date_to.strftime('%d-%b-%Y')) ,
                        'request_status': 'new',
                })
                approval_request_id = self.env['approval.request'].sudo().create(request_list)
                vals ={
                        'user_id': line.incharge_id.company_id.hr_id.user_id.id,
    ...
    def action_approved(self):
        ...
        for line in self.attendance_line_ids:
            ...
            if line.days > 0:
                ...
                check_out = 0 
                count_day = 0
                tota_working_days=line.days                 
                if self.date_to.month==2: 
                    tota_working_days += 3      
                for attendance in range(round(tota_working_days)):                    
